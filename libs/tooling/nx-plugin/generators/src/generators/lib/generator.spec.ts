...
describe('lib generator', () => {
  ...
  describe('state', () => {
    it('should generate state library', async () => {
      ...
      await generator(tree, options);
      const config = readProjectConfiguration(tree, 'shared-state-example');
      expect(config).toBeDefined();
      expect(tree.exists('libs/shared/state/example/package.json')).toBeFalsy();
      expect(tree.exists('libs/shared/state/example/README.md')).toBeFalsy();
      expect(
        tree.exists(
